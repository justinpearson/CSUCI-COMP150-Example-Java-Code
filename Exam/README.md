...
In 'main' part A, we declare two LabeledPoint objects and draw them.
...
Note: For ease of scanning, "cities_xy_nospaces.tsv" has underscores in city names: "New_York", etc. This makes it easier for you; otherwise Scanner.next() would choke on the spaces. However, when copying that name into your City object, you should replace the underscores with spaces. String has a 'replace' method that you can look up.


In 'main' part B, you are given code that loads the data from "cities_xy_nospaces.tsv" into a list of cities and plots them. 
...
In 'main' Part C, you are given code that makes an Itinerary object, adds 4 cities to it, and draws it.
...
In 'main' Part "Bonus", you are given code that makes an Itinerary object, adds the cities in the order that comprises the shortest tour, and draws it.

If the Itinerary class is working properly, the "Part Bonus" code in main should draw a picture like "CityMap-Part-Bonus.png".
